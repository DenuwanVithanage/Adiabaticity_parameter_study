...
for every available r_Li2 value for a selected Li2-X collision partner
...
# Hard-coded fitting window factor
WINDOW_FACTOR = 5.0
...
def fit_rho(R_bohr, V_cm1, E_collision_cm1,
            window_factor=WINDOW_FACTOR, min_points=MIN_POINTS):
    """
...
def analyze_one_r(data, partner, E_collision_cm1, r_selected_bohr,
                  mu_amu, v_m_s, v_cm_s):
    """
    For one r_Li2 value, calculate rho and xi for all available theta values.
    """
...
def main():
    ...
    print(f"Using hard-coded fitting window factor = {WINDOW_FACTOR:.3f}")
    print(f"Reduced mass mu = {mu_amu:.8f} amu")
    print(f"Relative velocity v = {v_m_s:.8f} m/s")
    print(f"Relative velocity v = {v_cm_s:.8f} cm/s")

    # Optional: create folder for output files
    output_dir = f"rho_xi_{partner}_E{E_collision_cm1:.0f}_all_r"
    ...
    for i, r_selected_bohr in enumerate(available_r, start=1):
        ...
        output_file = (
            f"{output_dir}/rho_xi_{partner}_"
            f"E{E_collision_cm1:.0f}_"
            f"r{i:02d}_{r_selected_bohr:.6f}bohr.csv"
        )
        ...
        print(
            ...
            f"({r_selected_angstrom:.6f} Å) -> {output_file}"
        )

    # Also save one combined file containing all r values
    combined_df = pd.concat(all_clean_tables, ignore_index=True)

    combined_file = (
        f"{output_dir}/rho_xi_{partner}_"
        f"E{E_collision_cm1:.0f}_all_r_combined.csv"
    )
