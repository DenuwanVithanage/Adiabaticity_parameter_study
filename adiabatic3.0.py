...
def make_combined_plots(combined_df, partner, E_collision_cm1, output_dir):
    """
    Make combined plots for one collision partner and one collision energy.
    ...
    if plot_df.empty:
        print("\nNo valid fitted points available for plotting.")
        return
    ...
    print(f"\nSaved plot: {xi_plot_file}")
    print(f"Saved plot: {rho_plot_file}")
...
def main():
    ...
    combined_file = os.path.join(
        output_dir,
        f"rho_xi_{partner}_"
        f"E{E_collision_cm1:.0f}_all_r_combined.tsv"
    )
